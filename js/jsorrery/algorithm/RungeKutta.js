	...
	function(ns, $) {
		...
		var RungeKutta = {
			...
			moveBody : function(body, deltaTIncrement, deltaTIncrementSquared, i){
				...
				for(var i=0; i<4; i++){
					var pos1 = body.position.clone();
					var vel1 = body.velocity.clone();
				 
					var pos2 = body.position.clone().add(vel1.clone().multiplyScalar(0.5*delta));
					var vel2 = body.velocity.clone().add(accel.clone().multiplyScalar(0.5*delta));
				 
					var pos3 = body.position.clone().add(vel2.clone().multiplyScalar(0.5*delta));
					var vel3 = body.velocity.clone().add(accel.clone().multiplyScalar(0.5*delta));
				 
					var pos4 = body.position.clone().add(vel3.clone().multiplyScalar(delta));
					var vel4 = body.velocity.clone().add(accel.clone().multiplyScalar(delta));
