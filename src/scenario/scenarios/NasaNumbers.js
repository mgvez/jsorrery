...
define([
	...
	return {
		get : function(orbitType, missionName){
			...
			var numbers = orbitData[missionName] && orbitData[missionName][orbitType];
			if(!numbers) return;

			var v = numbers.spaceVel * ns.FT_TO_M;
			var r = (numbers.altitude * ns.FT_TO_M) + (common.earth.radius * ns.KM);
			var C = (2 * GM) / (r * Math.pow(v, 2));

			var pathAngle = ns.DEG_TO_RAD * numbers.pathAngle;
			...
			var ln = Math.asin( Math.sin(ns.DEG_TO_RAD * numbers.geoLat) / Math.sin(ns.DEG_TO_RAD * numbers.incl) );
			var as = Math.atan( Math.tan(ln) * Math.cos(ns.DEG_TO_RAD * numbers.incl) );

			var argumentPerigee = ln - trueAnomaly;

			var geoLongAscNode = ((ns.DEG_TO_RAD * numbers.long) - as) + ns.CIRCLE;
			...
			if(numbers.gmt){
				...
				epoch = new Date(new Date(orbitData[missionName].launchTime).getTime() + numbers.time*1000);
			}
